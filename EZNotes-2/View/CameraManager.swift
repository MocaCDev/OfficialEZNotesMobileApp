...
class CameraManager: ObservableObject {
    ...
    @Published public var images_to_upload: ImagesUploads = ImagesUploads()
    ...
    private func setupVideoInput() {
        do {
            let camera = AVCaptureDevice.default(.builtInTripleCamera, for: .video, position: position)
    ...
    func captureImage() {
        sessionQueue.async { [weak self] in
            ...
            cameraDelegate = CameraDelegate { [unowned self] image in
                self.capturedImage = image
                self.images_to_upload.images_to_upload.append(["\(arc4random()).jpeg":self.capturedImage!])
            }
            
            if let cameraDelegate {
                self.photoOutput.setPreparedPhotoSettingsArray([photoSettings]) { (suc: Bool, err: Error?) -> Void in
                    if suc {
